...
def _scrapingant_get_json(url: str, token: str) -> list | dict:
    """
    Fetch a JSON API endpoint through ScrapingAnt.

    With browser=false ScrapingAnt acts as a plain HTTP proxy and returns
    the raw server response body directly (no JSON envelope, no HTML wrapping).
    This is exactly what we need for the Truth Social REST API.

    proxy_type=residential + proxy_country=US bypasses geo-restrictions.
    """
    params = {
        "url":           url,
        "x-api-key":     token,
    ...
    # With browser=false the response body IS the raw API response (JSON text)
    try:
        return resp.json()
    except Exception:
        raise ValueError(
            f"ScrapingAnt response is not valid JSON.\n"
